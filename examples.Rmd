...
## TCW 14.Jan.2024: Continued code reorganization
```
  
This is an [R Markdown](http://rmarkdown.rstudio.com) Notebook. When you execute code within the notebook, the results appear beneath the code. 

**NOTE** that several of the executable chunks that make changes to iNaturalist data are marked with the option 'eval=FALSE' to prevent their execution. If you wish to run these, change the option in the chunk header to 'eval=TRUE'.
  
Based on code from James Mickley, https://github.com/mickley/flora-of-oregon-inat
...
# Send messages
## message to a single , with markdown formatting:
...
Returns first page of messages received by logged-in user (box="inbox") 
...
Returns first page of messages sent by logged-in user (box="sent") to user_id.
...
out <- iNatAPI("GET", "v1", "observations/observers", authToken, 
...
out <- iNatAPI("GET", "v1", "identifications/identifiers", authToken, 
...
out <- iNatAPI("GET", "v2", "identifications/identifiers", authToken, 
    place_id = 10, observation_taxon_id = 211194, own_observation = "false", 
    is_change = "false", per_page=100,
